...
{
	private static final Logger LOG = LoggerFactory.getLogger(JilterProcessor.class);
...
//        log.debug(">SMFIC command is '" + ((char) this.packet.getCommand()) + "', Raw packet data:" + Util.newline() + Util.hexDumpLong(this.packet.getData()));
        ...
        switch (this.packet.getCommand())
        {
            ...
            case JilterConstants.SMFIC_DATA:
                LOG.debug("SMFIC_DATA");
                processDataPacket(writeChannel);
                break;
    ...
    private void processDataPacket(WritableByteChannel writeChannel)
            throws IOException
    {
        sendReplyPacket(writeChannel, this.handler.data());
    }
